...
export function ChatWidget() {
  ...
  const { messages, sendMessage, status, error, setMessages } = useChat({
    id: "esb-seo-chat",
    // Pourquoi : pas d'initialMessages ici — serveur et client démarrent
    // avec le même état vide, donc rien à comparer qui puisse diverger.
    transport: new DefaultChatTransport({ api: "/api/chat" }),
  });

  // Pourquoi : ce useEffect ne s'exécute qu'après hydratation (jamais côté
  // serveur, jamais pendant le rendu initial), donc charger le localStorage
  // ici ne peut plus provoquer de mismatch.
  useEffect(() => {
    ...
    <>
      <div className="fixed bottom-5 right-5 z-50 sm:bottom-8 sm:right-8">
        <Button
          ...
          aria-label={open ? "Fermer l'assistant ESB" : "Ouvrir l'assistant ESB"}
        >
          {open ? <X className="size-5" /> : <MessageCircle className="size-5" />}
          <span className="ml-2 hidden sm:inline">{open ? "Fermer" : "ESB Assistant"}</span>
        </Button>
      </div>
      ...
      >
        <header className="flex items-center gap-3 border-b border-border bg-secondary/60 px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate font-display text-sm font-semibold">ESB Assistant</p>
            <p className="truncate text-xs text-muted-foreground">Chatbot - réponse immédiate</p>
          </div>
        ...
        <Conversation className="flex-1">
          <ConversationContent className="gap-4">
            {messages.length === 0 && (
              <div className="space-y-4 py-4">
                <p className="text-sm text-muted-foreground">
                  Bonjour 👋 Posez votre question sur le référencement de votre site : audit,
                  positionnement Google, visibilité dans les IA (ChatGPT, Perplexity, Gemini) ou
                  création d&apos;agents de prospection IA.
                </p>
            ...
            {status === "submitted" && <Shimmer className="text-sm">Analyse en cours...</Shimmer>}
            {error && (
